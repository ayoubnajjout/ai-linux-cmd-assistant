...
export default function Auth({ darkMode, darkModeString }) {
  // Ensure we have a boolean darkMode value regardless of how it's passed
  const isDarkMode = darkModeString === "true" ? true : 
                    darkModeString === "false" ? false : 
                    typeof darkMode === 'boolean' ? darkMode : 
                    localStorage.getItem('linuxAssistantDarkMode') === 'true';
  ...
  return (
    <div className={`min-h-screen flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'}`}>
      <div className={`max-w-md w-full mx-auto rounded-2xl overflow-hidden shadow-xl ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
        ...
        <div className={`p-8 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`}>
          ...
          {error && (
            <div className={`mb-4 p-3 rounded ${darkMode ? 'bg-red-900 text-red-200' : 'bg-red-100 text-red-800'}`}>
              {error}
            </div>
          )}
